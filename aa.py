...
def update_lines(num, dataLines, lines,pts):
    ...
    for line, pt, data in zip(lines,pts, dataLines):
        ...
        z=data[2,:num]
        if (num>1):
            if(x[-1]**2+y[-1]**2+z[-1]**2 < boundary**2):
                line.set_linestyle('--')
                pt.set_marker('D')
            if(x[-1]**2+y[-1]**2+z[-1]**2 >= boundary**2):
                line.set_linestyle('-')
...
data = [Gen_RandLine(30000, 3) for index in range(number_of_particles)]
...
ax.set_xlim3d([-0.1, 8.0])
ax.set_xlabel('X')
#ax.grid(False)
#ax.axis('off')
ax.set_ylim3d([-0.1, 8.0])
ax.set_ylabel('Y')

ax.set_zlim3d([-0.1, 8.0])
ax.set_zlabel('Z')

ax.set_title('3D Test')
# Creating the Animation object
line_ani = animation.FuncAnimation(fig, update_lines, frames =3000, fargs=(data, lines, pts),
                                   interval=50, blit=False)
line_ani.save('3d_multiple.mp4', fps=15, extra_args=['-vcodec', 'libx264'])
