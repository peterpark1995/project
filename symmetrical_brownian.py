...
def brownian(p, n, dt, out=None):
    ...
    #Here we change our random variables to have integer value
        if (p[1] <= boundary or p[0] <= boundary) and (not boundary_reached):
            r_x=norm.rvs(size= 1, scale=delta*sqrt((p[0]**3)*p[1])*sqrt(dt))
            r_y=norm.rvs(size= 1, scale= delta*sqrt(p[0]**3*p[1])*sqrt(dt))
        #update our points
            p[0] = p[0] + r_x
            p[1] = p[1] + r_y
            output[0,i] = p[0]
    ...
    #Here we change our random variables to have integer value
        ...
        n=n-1
        i=i+1
        if p[0] > boundary or p[1] > boundary:
            boundary_reached=True
    indicator=0
    for num in range(end,0, -1):
        if output[0,num] <epsilon or output[1,num] <epsilon :
            indicator=num
    if (boundary_reached):
